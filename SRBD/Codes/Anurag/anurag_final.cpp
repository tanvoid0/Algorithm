...
void solvecase()
{
    ...
    while (!isEmpty())
    {
        ...
        if (l > 1)
        {
            if (x - 1 >= 0 && goUp(x, y) && goDown(x - 1, y) && !vis[x - 1][y])
            {
                vis[x - 1][y] = 1;
                push(x - 1, y, l - 1);
            }
            if (x + 1 < N && goDown(x, y) && goUp(x + 1, y) && !vis[x + 1][y])
            {
                vis[x + 1][y] = 1;
                push(x + 1, y, l - 1);
            }
            if (y - 1 >= 0 && goLeft(x, y) && goRight(x, y - 1) && !vis[x][y - 1])
            {
                vis[x][y - 1] = 1;
                push(x, y - 1, l - 1);
            }
            if (y + 1 < M && goRight(x, y) && goLeft(x, y + 1) && !vis[x][y + 1])
            {
                vis[x][y + 1] = 1;
                push(x, y + 1, l - 1);
            }
